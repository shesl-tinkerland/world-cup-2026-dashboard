...
- **Persistence is preference-only and storage-safe.** Just two display preferences persist — the selected timezone (`state.tz`) and the watch-party toggle (`state.party`) — written to `localStorage` under `wc26-prefs` via the guarded `loadPrefs`/`savePrefs` helpers. Both wrap storage in `try/catch` so they **silently no-op when storage is blocked** (e.g. a sandboxed artifact frame), preserving the in-memory-only behavior there. All other state (search, team, round, Today/Upcoming) stays in memory by design — restoring transient filters on return reads as "missing matches." On first visit `detectTz()` maps the browser's IANA zone to a named dropdown option (else `"Auto"`, the browser's own zone); precedence is **saved → detected → "Eastern"**. Don't reach for `sessionStorage` or persist additional filters without a deliberate reason.
